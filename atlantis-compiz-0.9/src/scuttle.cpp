...
void
AtlantisScreen::CrabPilot (int index)
{
    crabRec * crab = &(mCrab[index]);
    ...
    float bottom = getGroundHeight (x, y);

    if (z > bottom) { /* falling */
	float fallDist = mSpeedFactor * crab->size / 5;
	z -= fallDist;
	...
	float factor = mSpeedFactor * (1 - moveAmount);
	float ang, temp;
	...
	if (crab->scuttleAmount <= 0)
	{
	    ...
	    crab->scuttleTheta= 0;
	    crab->scuttleAmount = (int) ((randf (30) + 7) / mSpeedFactor);
	...
	{
	    float cosAng = cosf (fmodf (i * mArcAngle * toRadians - ang,
	                                2 * PI));
	    if (cosAng <= 0)
		continue;

	    float d = (mSideDistance - 0.75 * crab->size) / cosAng;
	    float d2 = hypotf (x, y);
