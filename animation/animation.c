/* Markings:
 * XXX MOD needs modularising
 * XXX TRANS needs putting into transitions plugin
 * XXX ANIM stays here
/**
...
static const CompMetadataOptionInfo animScreenOptionInfo[] = {
    ...
    { "magic_lamp_grid_res", "int", "<min>2</min>", 0, 0 },
...
animEnsureModel(CompWindow * w)
{
	...
	(isShadeUnshadeEvent != wasShadeUnshadeEvent) || // XXX Mod?
...
static void cleanUpParentChildChainItem(AnimScreen *as, AnimWindow *aw)
{
    ...
    aw->isDodgeSubject = FALSE; // XXX MOD
	...
	for (w = s->windows; w; w = w->next)
	{
		...
		if (aw->com.model &&
		    (aw->com.model->winWidth != WIN_W(w) ||
		     aw->com.model->winHeight != WIN_H(w))) // MOD
		{
		    // model needs update
		    // re-create model
		    if (!animEnsureModel (w))
...
static void animHandleEvent(CompDisplay * d, XEvent * event)
{
	...
	if (w)
	{
		...
		if (chosenEffect != AnimEffectNone)
		{
		    ...
		    if (aw->com.curAnimEffect->properties.setIconSizeFunc)
		        (*aw->com.curAnimEffect->properties.setIconSizeFunc)
							(w,
							 &aw->com.icon);
	...
	break;
    case ConfigureNotify: // MODULARIZE
    {
...
AnimEffect AnimEffectMagicLamp = &(AnimEffectInfo)
    {"animation:Magic Lamp",
     ...
     {.animStepFunc		= fxMagicLampModelStep,
      ...
      .initGridFunc		= fxMagicLampInitGrid,
      .updateBBFunc		= modelUpdateBB,
      .cleanupFunc		= fxMagicLampCleanup,
      .setIconSizeFunc		= fxMagicLampSetIconSize,
      .useQTexCoord		= TRUE}};
...
AnimEffect AnimEffectVacuum = &(AnimEffectInfo)
    {"animation:Vacuum",
     ...
     {.animStepFunc		= fxMagicLampModelStep,
      ...
      .updateBBFunc		= modelUpdateBB,
      .cleanupFunc		= fxMagicLampCleanup,
      .setIconSizeFunc		= fxVacuumSetIconSize,
      .useQTexCoord		= TRUE}};
